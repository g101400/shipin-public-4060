...
(function (global) {
  ...
  async function strategyCall(prompt, opts) {
    ...
    for (const id of order) {
      ...
      if (!m) continue;
      try {
        const r = await callOne(m, prompt, opts);
        saveSettings(s);
        return r;
      } catch (e) {
        lastErr = e;
  ...
  function pickRecord(dm, cb) {
    ...
    let q = "";
    // v2.4.1：占位符清晰 + 试试搜 chips（贴近感知：摄像机/NVR/监控点）
    ...
    const render = () => {
      ...
      const html =
        `<div class="field"><input id="pkSearch" class="inp" placeholder="输入名称 / 关键字搜索监控点（如：北台上所球机 / 摄像机离线 / NVR 配置）" value="${esc2(q)}"></div>` +
