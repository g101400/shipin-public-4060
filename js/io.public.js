...
(function (global) {
  ...
  async function inflateRaw(bytes) {
    if (typeof DecompressionStream === "undefined")
      throw new Error("当前环境不支持解压(deflate)，请用 KML 文本导入");
  ...
  function unzip(buf) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (dv.getUint32(0, true) !== 0x04034b50) throw new Error("不是有效的 zip 文件");
        const files = {};
        let off = 0;
        while (off + 4 <= buf.byteLength) {
          ...
          const name = strFromUtf8(buf.subarray(off + 30, off + 30 + nameLen));
          ...
          // 跳到下一个文件（用中心目录定位更稳，这里用本地头近似）
          off = off + 30 + nameLen + extraLen + compSize;
          // 防御：越界
          if (compSize === 0 && name === "") break;
        }
        resolve(files);
      } catch (e) { reject(e); }
    });
  ...
  // 流式解压：逐条目回调 onEntry(name, bytes)，不把所有条目同时驻留内存（防大包 OOM/死机）
  // 与 unzip 走同一套本地头遍历逻辑，但每处理完一个条目即释放其字节，仅持有当前条目
  async function unzipStream(buf, onEntry) {
    ...
    while (off + 4 <= buf.byteLength) {
      ...
      let data;
      try {
        if (method === 0) data = buf.subarray(off + 30 + nameLen + extraLen, off + 30 + nameLen + extraLen + compSize);
        else if (method === 8) data = await inflateRaw(buf.subarray(off + 30 + nameLen + extraLen, off + 30 + nameLen + extraLen + compSize));
        else throw new Error("不支持的压缩方式:" + method);
        await onEntry(name, data);
      } finally {
        data = null; // 释放本条目字节，便于 GC（关键：避免整包字节同时驻留）
      }
      off = off + 30 + nameLen + extraLen + compSize;
      if (compSize === 0 && name === "") break;
    }
  ...
  function unzipCount(buf) {
    ...
    if (buf.byteOffset !== 0 || buf.byteLength !== buf.buffer.byteLength) buf = buf.slice();
    const dv = new DataView(buf.buffer);
    if (dv.getUint32(0, true) !== 0x04034b50) return 0;
    let off = 0, n = 0;
    while (off + 4 <= buf.byteLength) {
      const sig = dv.getUint32(off, true);
      if (sig !== 0x04034b50) break;
      ...
      const extraLen = dv.getUint16(off + 28, true);
      const name = strFromUtf8(buf.subarray(off + 30, off + 30 + nameLen));
      if (/\.(jpe?g|png|gif|bmp|webp)$/i.test(name)) n++;
      off = off + 30 + nameLen + extraLen + compSize;
      if (compSize === 0 && name === "") break;
    }
    return n;
  ...
  function recordToPlacemark(r) {
    ...
    push("office", r.office); push("station", r.station); push("btype", r.btype);
...
<kml xmlns="http://www.opengis.net/kml/2.2" xmlns:gx="http://www.google.com/kml/ext/2008">
  <Document>
    ...
    <name>${escapeXml(root || "视频设备运维基础信息")}</name>
${places}
  </Document>
</kml>`;
  }
  ...
  function parseKmlToRecords(kmlText) {
    ...
    for (const pm of pms) {
      ...
      // 奥维文件夹层级：最近 Folder=管理段（可含「段--类型」），再上层 Folder=机构（以 所/站 结尾）
      const folderNames = [];
  ...
  async function importKmzBuffer(buf) {
    ...
    for (const r of recs) {
      ...
      for (const ref of (r.photoFiles || [])) {
        ...
        if (data && !seen.has(data)) { seen.add(data); r.photos.push({ caption: ref.split("/").pop(), dataUrl: "data:image/jpeg;base64," + bytesToB64(data) }); }
      }
    }
    return recs;
  }
  ...
  function parseJsonToRecords(text) {
    ...
    for (const o of arr) {
      ...
      out.push({
        ...
        office: o.office || o.机构 || "", station: o.station || o.库渠 || "",
  ...
  function buildChaohe(records, cols) {
    const C = [
      ...
      { k: "office", t: "机构", g: (r) => r.office },
      { k: "station", t: "库渠", g: (r) => r.station },
      { k: "btype", t: "摄像机类型", g: (r) => r.btype },
  ...
  // CSV 导出：与「视频设备运维」属性表（xlsx）格式对齐（独立列，非 folder 编码），
  ...
  // ---------- 统一属性表（监控点导入导出 xls/csv/xlsx 共用，保证往返一致）----------
  const NL = String.fromCharCode(10);
  const ATTR_COLS = [
    { k: "name", t: "名称", g: function (r) { return r.name || ""; } },
    { k: "mgmt", t: "管理处", g: function (r) { return r.mgmt || ""; } },
    { k: "office", t: "机构", g: function (r) { return r.office || ""; } },
    { k: "station", t: "库渠", g: function (r) { return r.station || ""; } },
    { k: "btype", t: "摄像机类型", g: function (r) { return r.btype || ""; } },
  ...
  function xlsxBytesFromMatrix(rows) {
    ...
    var workbook = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + NL +
      '<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="视频设备运维" sheetId="1" r:id="rId1"/></sheets></workbook>';
  ...
  function xlsBytesFromMatrix(rows) {
    ...
    var xml = '<?xml version="1.0"?>' + NL + '<?mso-application progid="Excel.Sheet"?>' + NL +
      '<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="视频设备运维"><Table>' + body + '</Table></Worksheet></Workbook>';
    return utf8(xml);
  }
  ...
  function matrixToRecords(matrix) {
    ...
    for (var r = 1; r < matrix.length; r++) {
      ...
      var office = gi(row, ["机构", "管理所", "城市"]);
      var station = gi(row, ["库渠", "管理站", "区县"]);
      var btype = gi(row, ["摄像机类型", "建筑物类型", "古建类型", "类型"]);
      var folder = gi(row, ["文件夹"]);
      if (!office && folder) {
        var fp = folder.split("/").filter(Boolean);
        office = fp[1] || office;
        var seg = fp[2] || "";
        if (seg.indexOf("--") >= 0) { var a = seg.split("--"); if (!station) station = a[0]; if (!btype) btype = a[1]; }
        else if (!btype) btype = seg;
      }
      ...
      out.push({
        ...
        params: params, description: descText, photos: [], base: false, custom: true
      });
    }
    return out;
  }
  ...
  // 文本格式：首行可为中文表头（名称/机构/库渠/摄像机类型/经度/纬度/说明），兼容增删列与超集；
  // 无表头时严格按 `名称,经度,纬度[,机构,摄像机类型,说明]` 顺序逐行解析。
  // 列名与 shipin 自身 matrixToRecords 对齐（机构/库渠/摄像机类型），确保原样再导入不丢字段。
  ...
  function recordsToOvobj(records, cols) {
    ...
    var esc = function (s) { return '"' + String(s == null ? "" : s).replace(/"/g, '""') + '"'; };
    var lines = ["# 视频设备运维一张图 ovobj 文本坐标文件（名称/机构/库渠/摄像机类型/经度/纬度/说明）",
  ...
  // 布局：OviO(4) + 0(8) + u32 0x69 + 0(8) + u32 文件大小 + 数据区（点 = double lat + double lon + u32 图标 + Pascal 串名称）
  ...
  function parseOvobjBinary(bytes) {
    ...
    for (var k = 0; k < hits.length; k++) {
      ...
      out.push({
        ...
        params: icon != null ? { 图标: icon } : {}, description: "", photos: [], base: false, custom: true
      });
    }
    return out;
  }
  function recordsToOvobjBinary(records) {
    ...
    out.set(utf8("OviO"), 0);
    out.set(_u32le(0x69), 12);
    out.set(_u32le(total), 24);
    out.set(bodyBytes, 28);
    return out;
  }
  ...
  global.IO = {
    ...
    // 导出入口
    exportOvkmz(records, root) { downloadBytes((root || "视频设备运维基础信息") + ".ovkmz", recordsToKmzBytes(records), "application/vnd.google-earth.kmz"); },
    ...
    exportAttrCsv(records, root, cols) { downloadText((root || "视频设备运维信息") + ".csv", buildAttrCsv(records, cols), "text/csv;charset=utf-8"); },
    exportXlsx(records, root, cols) { downloadBytes((root || "视频设备运维信息") + ".xlsx", buildAttrXlsx(records, cols), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"); },
    exportXls(records, root, cols) { downloadBytes((root || "视频设备运维信息") + ".xls", buildAttrXls(records, cols), "application/vnd.ms-excel"); },
    // ovobj/obj 文本坐标（奥维坐标 · 纯文本往返）
    parseOvobjToRecords, recordsToOvobj, parseOvobjBinary, recordsToOvobjBinary,
    exportOvobj(records, root, cols) { downloadText((root || "视频设备运维信息") + ".ovobj", recordsToOvobj(records, cols), "text/plain;charset=utf-8"); },
    exportOvobjBin(records, root) { downloadBytes((root || "视频设备运维信息") + ".ovobj", recordsToOvobjBinary(records), "application/octet-stream"); },
  };
})(window);
